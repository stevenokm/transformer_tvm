...
target = tvm.target.Target(
    "cuda -arch=sm_61"
)  # NOTE: the setting is for GTX 1080 ti or later, reffer to https://developer.nvidia.com/cuda-gpus
...
lib_file = "%s-%s-B%d-%s-lib.so" % (network, layout, batch_size, dtype)
...
# or need to compile with "DynamicToStatic" pass
if dynamic_to_static:
    if os.path.exists(lib_file):
        logging.info("Load lib...")
        lib = tvm.runtime.load_module(lib_file)
    else:
        logging.info("Compile with dynamic to static...")
        with auto_scheduler.ApplyHistoryBest(log_file):
            with tvm.transform.PassContext(
                opt_level=3,
                config={"relay.backend.use_auto_scheduler": True},
                required_pass=["DynamicToStatic"],
            ):
...
if dynamic_to_static:
    if torch.cuda.device_count() > 1:
        # create pipeline_executor for multi-gpu
        logging.info("Create pipeline_executor...")
        logging.warning("pipeline_executor is working in progress...")
        exit(1)
    else:
        # create graph_executor for single-gpu
        logging.info("Create graph_executor...")
        # Create graph executor
        dev = tvm.device(str(target), 0)
        module = graph_executor.GraphModule(lib["default"](dev))
        for input_name in input_names:
            input_data = tvm.nd.array(model_inputs[input_name].numpy().astype(dtype))
            module.set_input(input_name, input_data)

        # Evaluate
        logging.info("Evaluate inference time cost...")
        logging.info(module.benchmark(dev, repeat=3, min_repeat_ms=500))
else:
    # Create VM runtime
